...
def PBFT_prepare_consensus(nodes, primary_node_id, error_rate):
    prepare_nodes = []
    ...
    for i in range(len(nodes)):
        ...
        if error == 0:
            ...
            if x > nodes[i] and correct > error_rate:
                prepare_nodes.append(i)
            elif (x <= nodes[i] or correct <= error_rate) and i == primary_node_id:
                primary_node_wrong = True
    return len(prepare_nodes) >= (2*f+1), len(prepare_nodes), primary_node_wrong
    
def PBFT_commit_consensus(nodes, primary_node_id, error_rate):
    commit_nodes = []
    primary_node_wrong = False
    f = (len(nodes) - 1) // 3
    for i in range(len(nodes)):
...
def PBFT_run_consensus(nodes, error_rate):
    all_round = 10000
    correct_round = 0
    all_prepare_count = 0
    all_commit_count = 0
    ...
    for i in range(all_round):
        prepare_res, prepare_count, primary_node_wrong = PBFT_prepare_consensus(nodes, primary_node_id, error_rate)
        ...
        if prepare_res == True:
            commit_res, commit_count, primary_node_wrong = PBFT_commit_consensus(nodes, primary_node_id, error_rate)
    ...
    with open(r"./res_error/PBFT.txt", "a") as f:
        f.write(str(PBFT_consensus_success_rate) + " " + str(PBFT_message_right_rate) + " " + str(PBFT_primary_node_error_rate) + "\n")



def WPBFT_prepare_consensus(nodes, weights, primary_node_id, error_rate):
    prepare_nodes = []
    primary_node_wrong = False
    f = (len(nodes) - 1) // 3
    for i in range(len(nodes)):
...
def WPBFT_commit_consensus(nodes, weights, primary_node_id, error_rate):
    commit_nodes = []
    primary_node_wrong = False
    f = (len(nodes) - 1) // 3
    for i in range(len(nodes)):
...
def WPBFT_run_consensus(nodes, weights, error_rate):
    all_round = 10000
    correct_round = 0
    all_prepare_count = 0
    all_commit_count = 0
    ...
    for i in range(all_round):
        prepare_res, prepare_count, prepare_nodes, primary_node_wrong = WPBFT_prepare_consensus(nodes, weights, primary_node_id, error_rate)
        ...
        if prepare_res == True:
            commit_res, commit_count, commit_nodes, primary_node_wrong = WPBFT_commit_consensus(nodes, weights, primary_node_id, error_rate)
    ...
    # print("WPBFT 共识成功率: ", WPBFT_consensus_success_rate)
    # print("WPBFT 消息正确率: ", WPBFT_message_right_rate)
    # print("WPBFT 主节点出错率: ", WPBFT_primary_node_error_rate)

    with open(r"./res_error/WPBFT.txt", "a") as f:
...
def IWPBFT_prepare_consensus(nodes, weights, primary_node_id, error_rate):
    prepare_nodes = []
    primary_node_wrong = False
    f = (len(nodes) - 1) // 3
    for i in range(len(nodes)):
...
def IWPBFT_commit_consensus(nodes, weights, primary_node_id, error_rate):
    commit_nodes = []
    primary_node_wrong = False
    f = (len(nodes) - 1) // 3
    for i in range(len(nodes)):
...
def IWPBFT_run_consensus(nodes, weights, error_rate):
    all_round = 10000
    correct_round = 0
    all_prepare_count = 0
    all_commit_count = 0
    ...
    for i in range(all_round):
        prepare_res, prepare_count, prepare_nodes, primary_node_wrong = IWPBFT_prepare_consensus(nodes, weights, primary_node_id, error_rate)
        ...
        if prepare_res == True:
            commit_res, commit_count, commit_nodes, primary_node_wrong = IWPBFT_commit_consensus(nodes, weights, primary_node_id, error_rate)
    ...
    with open(r"./res_error/IWPBFT.txt", "a") as f:
...
def start_run_with_error_rate(nodes, error_rate):
    weights = calc_weights(nodes)
    
    IWPBFT_nodes = nodes.copy()
    IWPBFT_run_consensus(IWPBFT_nodes, weights, error_rate)

    WPBFT_nodes = nodes.copy()
    WPBFT_run_consensus(WPBFT_nodes, weights, error_rate)
    
    PBFT_nodes = nodes.copy()
    random.shuffle(PBFT_nodes)    
    PBFT_run_consensus(PBFT_nodes, error_rate)



if __name__ == "__main__":
    with open("./node/nodes_high.txt", "r") as f:
        data = f.readlines()
    ...
    for line in data:
        if count%5 == 4:
            assert len(eval(line.strip())) == 58
            nodes_list.append(eval(line.strip()))
        count += 1
    # print(len(nodes_list))
    error_rates = [0, 0.02, 0.04, 0.06, 0.08, 0.1, 0.12, 0.14, 0.16, 0.18, 0.2]
    for nodes in nodes_list:
        for error_rate in error_rates:
            start_run_with_error_rate(nodes, error_rate)
