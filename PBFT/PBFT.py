...
def run_PBFT(nodes,proportion,checkpoint_frequency0,clients_ports0,timer_limit_before_view_change0): # All the nodes participate in the consensus
    ...
    print(similarity)
    # similarity = [0.15, 0.02, 0.01, 0.06, 0.34, 0.01, 0.09, 0.64, 0.12, 0.08, 0.29, 0.01, 0.04, 0.74, 0.03, 0.03, 0.74, 0.15, 0.11, 0.18, 0.01, 0.13]
...
class Node():
    ...
    def process_received_message(self,received_message,waiting_time):
            ...
            if (message_type=="REQUEST"):
                # print("request is", received_message)
                ...
                if (client_id not in self.last_reply_timestamp or timestamp > self.last_reply_timestamp[client_id]): # The request is only processed if its timestamp is greater than teh timestamp of the last request that the node responded to
                    if (self.node_id==self.primary_node_id):
                        ...
                        if ((last_timestamp<actual_timestamp)or(last_timestamp==actual_timestamp and (received_message["request"] != reply["request"] for reply in self.message_reply))):
                            ...
                            self.broadcast_preprepare_message(request_message=received_message,nodes_ids_list=consensus_nodes, flag=False)
                ...
                if(digest != requests_digest): # todo: 这里preprepare是主节点发出来的，应该viewchange并且对主节点处罚
                    # print("wrong message", received_message)
                    # print("wrong message from primary, need to view change")
                    # print(self.view_number)
                    self.lock = True
                    self.broadcast_view_change()
                    # 由于多进程实现的问题，view change实际上存在问题...暂时用sleep来代替view change的过程
                    # todo: 需要设置一个全局变量用于切换主节点，否则其他节点不清楚主节点(虽然其他节点也会受到错误消息，但是时间不同)
                    # time.sleep(2)

                    # todo: 设置一个global变量存放当前的request, 在view切换完成之后, 由下一个主节点再次preprepare
                ...
                node_id = received_message["node_id"]
                if ((received_message["view_number"]==self.view_number) and (digest==requests_digest)): 
                    ...
                    for message in self.message_log:
                        aa = ""
                        try:
                            aa = message["message_type"]
                        except:
                            pass
                        if (aa!="" and (message["message_type"]=="PREPREPARE") and (message["view_number"]==received_message["view_number"]) and (message["sequence_number"]==received_message["sequence_number"]) and (message["request"]==received_message["request"])):
                            p = 1
                            break
                ...
                if (self.view_number == received_message["view_number"] and (digest==requests_digest)):
                    self.message_log.append(received_message)
                    tuple = (received_message["view_number"],received_message["sequence_number"],received_message["request_digest"])
                ...
                if (new_asked_view % len(consensus_nodes) == self.node_id): # If the actual node is the primary node for the next view
                    # print(f"I'm the next primary node {self.node_id}, I start to work. Now len is {len(self.received_view_changes[new_asked_view])}")
                    ...
                    if len(self.received_view_changes[new_asked_view])==2*f:
                        ...
                        if new_asked_view not in self.asked_view_change:
                            ...
                            # # todo: 按照check里的那样解开,否则会有bytes字样
                            # # or 修改broadcast_view_change的返回值,返回原生数据 √
                            # [view_change_message,public_key] = view_change_message.split(b'split')
                            # # Create a VerifyKey object from a hex serialized public key    
                            # verify_key = VerifyKey(public_key)   
                            # view_change_message  = verify_key.verify(view_change_message).decode()
                            # view_change_message = view_change_message.replace("\'", "\"")
                            # view_change_message = json.loads(view_change_message)
                        ...
                        try:    # 这一句不知道出了什么问题，有时候报错有时候不报，怪，先try着
                            ...
                            print("some error here", self.message_log[-1])
                            sequence_numbers_in_prepare_messages = []
                        ...
                        # Sign the message with the signing key
                        signed_view_change = signing_key.sign(str(new_view_message).encode())
                        ...
                        new_view_message = signed_view_change + (b'split')+  public_key
                        ...
                        if len(waiting_for_preprepare)>0:
                            for message in waiting_for_preprepare:
                                self.broadcast_preprepare_message(message, consensus_nodes, True)
                ...
                if len(O)!=0:
                    for message in O:
                        # if (received_message["request_digest"]!="null"): # 这里不应该是received_message
                        if (message["request_digest"]!="null"):
    ...
    def broadcast_preprepare_message(self,request_message,nodes_ids_list, flag): # The primary node prepares and broadcats a PREPREPARE message
        if (replied_requests[request_message["request"]]==0):
            ...
            self.preprepares[tuple]=digest
            self.message_log.append(preprepare_message)
            # print("here is preprepare broadcast", self.message_log[-1])

            preprepare_message_bak = preprepare_message
            # if flag == True:
            #     print(f"preprepare message is {preprepare_message_bak} and preprepare_message is {preprepare_message}")
    ...
    def broadcast_prepare_message(self,preprepare_message,nodes_ids_list): # The node broadcasts a prepare message
        if (replied_requests[preprepare_message["request"]]==0):
            ...
            prepare_message_bak = prepare_message
            # Generate a new random signing key
            signing_key = SigningKey.generate()
