...
def run_WPBFT(nodes,proportion,checkpoint_frequency0,clients_ports0,timer_limit_before_view_change0): # All the nodes participate in the consensus
...
def update_weights():
    ...
    global weights
    correct_count  = len(consensus_nodes) - len(malicious_nodes)
    R_c = correct_count / len(consensus_nodes)
    new_all_weight = 1 + R_c * (correct_count - 1)
    for node_id in consensus_nodes:
        if node_id in malicious_nodes:
            weights[node_id] = (weights[node_id] - R_c*weights[node_id]) / new_all_weight
        else:
            weights[node_id] = (weights[node_id] + R_c*(1 - weights[node_id])) / new_all_weight
...
def update_weights_while_primary_change(primary_node_id):
    ...
    tmp[primary_node_id] = tmp[primary_node_id] * 0.5
    for i in the_nodes_ids_list:
        if i!=primary_node_id:
            tmp[i] += deducted_weight/(len(the_nodes_ids_list)-1)
